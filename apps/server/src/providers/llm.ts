...
export class MockLLMProvider implements LLMProvider {
  ...
  async planUserIntent(input: UserIntentInput): Promise<UserIntentPlan> {
    ...
    return {
      ...
      dj: speak
        ? { speak: true, text: `Robot Radio redirects the signal: ${directive.description}.` }
        : { speak: false }
    };
  }
  ...
  async planContinuity(input: ContinuityInput): Promise<ContinuityPlan> {
    ...
    return {
      nextTrack: {
        description: `A fresh variation of ${intent.description}`,
        styles: intent.styles,
        mood: intent.mood,
        energy: intent.energy ?? 0.6,
        bpm: intent.bpmRange ? Math.round((intent.bpmRange[0] + intent.bpmRange[1]) / 2) : 116,
