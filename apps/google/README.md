...
```text
apps/google/web     React, station reducer, runtime, Web Audio, and debugger
apps/google/server  Native Node HTTP, WebSocket streams, and provider adapters
apps/google/shared  Google-application contracts and Zod schemas
```
...
pnpm --filter @robot-radio/google-server profile:google-tts -- --runs 3 --warmups 1
```
...
If you need failure or starvation controls, copy `apps/google/.env.example` to `.env` at the repository root. The server reads these values from its process environment.
