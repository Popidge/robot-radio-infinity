...
export class OpenAILLMProvider implements LLMProvider {
  ...
  planInitialIntent(input: InitialIntentInput): Promise<ProducerPlan> {
    return this.structured({
      ...
      effort: "low",
      system: `${PRODUCER_POLICY} Turn the opening request into a durable MusicalIntent and one concrete original first track. Energy values are decimals from 0 to 1. ${MUSIC_POLICY} Treat the composition plan as a radio cue sheet: set durationMs to 180000 and make the section durationMs values sum exactly to 180000; provide 3-7 sections, identify a genuinely instrumental presenter ramp before the first vocal, and include a clean bed or precise ending when musically appropriate. The final section must explicitly identify the ending as cold, resolved, or fade and state its exact tail behavior. For vocal sections, write concise original lyrics with one singable line per newline. For instrumental sections, omit lyrics completely. Use suggestedTiming opening. An optional cue must use purpose opening and be short enough for the authored ramp. Capture only durable listener details in memoryUpdates and provide a concise production fingerprint.`,
      input
    });
  }
  ...
  planContinuity(input: ContinuityInput): Promise<ProducerPlan> {
    return this.structured({
      ...
      schema: producerPlanSchema,
      effort: "low",
      system: `${PRODUCER_POLICY} Choose the next original track inside the current MusicalIntent and musical thesis. This is reactive personal radio, not a format clock. In interactive mode remain tightly listener-led. In cruise mode preserve the core thesis while changing one dominant musical dimension. In exploratory mode preserve two clear anchors from the listener's intent but make one bolder adjacent move by changing at least two dominant dimensions among rhythmic language, instrument family, harmonic color, vocal structure, tempo band, and spatial production; revive an earned callback or ask one light question only when it adds value, and never pretend the listener requested the move. Vary instrumentation, rhythm, hook shape, structure, and production against recentProductionFingerprints. ${MUSIC_POLICY} Set durationMs to 180000 and make the 3-8 section durationMs values sum exactly to 180000. Include explicit instrumental intro, break, or outro windows where musically appropriate. The final section must explicitly identify the ending as cold, resolved, or fade and state its exact tail behavior. For vocal sections, write concise original lyrics with one singable line per newline; for instrumental sections, omit lyrics. Mark clean speech beds in section descriptions. Use suggestedTiming continuity. Autonomous presentation should be sparse rather than absent: omit onAirCue in interactive continuity and whenever autonomy.speechSilenceMs is below 480000. In cruise or exploratory mode after at least 480000ms without speech, consider one fresh back_announce when the current track offers a specific truthful detail; still omit it when the link would be generic, and never speak merely because a threshold elapsed. The reducer independently enforces this interval. Use handoff_setup only when the supplied context establishes a committed next track, and mid_track_observation only for a rare specific insight worth crossing the music for.`,
      input
    });
  }
